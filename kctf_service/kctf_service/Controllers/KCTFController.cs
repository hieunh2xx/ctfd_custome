...
namespace kctf_service.Controllers
{
    ...
    public class KCTFController : ControllerBase
    {
        ...
        [HttpPost("/startchallenge/{challengeFolderName}")]
        public IActionResult StartChallenge(string challengeFolderName)
        {
            // Create a new process
            Process process = new Process();
            StringBuilder arguments = new StringBuilder("");
            arguments.Append("cd .. && cd ctf-directory && source kctf/activate");
            arguments.Append($" && cd {challengeFolderName} && kctf chal start");
            // Set the process start info
            process.StartInfo.FileName = "/bin/bash";
            process.StartInfo.Arguments = $"-c \"{arguments}\"";
            process.StartInfo.RedirectStandardOutput = true; // Capture the output
